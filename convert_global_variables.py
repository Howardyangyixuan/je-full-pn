...
if len(sys.argv) > 1:
	cate = sys.argv[1]
	g_lfd_hog_features_file = os.path.join(g_data_folder, 'shape_embedding/mat/%s.mat' % cate)

	# Pairwise distance
	g_shape_distance_matrix_file_mat = os.path.join(g_data_folder, 'shape_embedding/distance/%s.mat' % cate)
	g_shape_distance_matrix_file_txt = os.path.join(g_data_folder, 'shape_embedding/distance/%s.txt' % cate)

	# Embedding space
	g_shape_embedding_space_file_mat = os.path.join(g_data_folder, 'shape_embedding/space/%s.mat' % cate)
	g_shape_embedding_space_file_txt = os.path.join(g_data_folder, 'shape_embedding/space/%s.txt' % cate)
